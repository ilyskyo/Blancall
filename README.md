...
用户自配 API，支持任意 OpenAI 兼容接口（DeepSeek / 通义 / GLM / Kimi 等），填 Key 即用。联网搜索核验让提问自动联网搜索（Tavily），基于最新权威信息回答并附来源。学习数据深度接入让 AI 知道你哪里总错，能针对性出题、分析错误原因。跨文对话和历史续聊支持多篇文章一起问 AI，对话自动存档随时接着聊。
